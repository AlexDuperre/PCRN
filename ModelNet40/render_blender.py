...
# parallelizing for a whole datatset
# find . -name *.obj -print0 | xargs -0 -n1 -P3 -I {} blender --background --python render_blender.py
#
...
output_folder = os.path.dirname(args.obj)
import bpy
...
# import off file
bpy.ops.import_mesh.off(filepath=args.obj)

# scaling
mesh = bpy.context.object
maxDim = max(mesh.dimensions)
mesh.dimensions = mesh.dimensions/maxDim

# Centering
bpy.ops.object
#ob.origin_set(type='GEOMETRY_ORIGIN', center='MEDIAN')
...
print(os.path.split(os.path.splitext(args.obj)[0])[1])
model_identifier = os.path.split(os.path.splitext(args.obj)[0])[1]
fp = os.path.join(output_folder, model_identifier)
...
for i in range(0, args.views):
    ...
    filepath = fp + '_r_{0:03d}'.format(int(i * stepsize))
